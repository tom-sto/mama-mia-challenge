...
class RiskFactorPrediction(nn.Module):
    ...
    def maskRiskFactors(self, riskFactors: list[torch.Tensor], pred: torch.Tensor, continuousIndices: list[int] = [0]):
        ...
        for contIdx in continuousIndices:
            mask[contIdx] = (riskFactors[contIdx] != -1).any().item()       # continuous variables not one-hot encoded, so look for missing value of -1
        loss = []
        out = []
        idx = 0
